 ...
 */
const Sidebar = ({ isMobileOpen, isCollapsed, toggleCollapse, onClose }) => {
    ...
    >
      ...
      <div>
        <div className="h-16 flex items-center justify-between px-6 border-b border-darkBorder">
          ...
          >
            {showFull ? <ChevronLeft className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />}
          </button>
        </div>
      ...
      <div className="p-4 border-t border-darkBorder space-y-4">
        {user && (
          <div className={`flex items-center space-x-3 ${showFull ? '' : 'justify-center'}`}>
